...
public class CharacterManager : Singleton<CharacterManager>
{
    ...
    public IEnumerable<ActabilityTemplate> GetActabilityTemplates()
    {
        return _actabilities.Values;
    }
    ...
    public void Create(GameConnection connection, string name, byte race, byte gender, uint[] body, UnitCustomModelParams customModel, byte ability1, byte ability2, byte ability3, byte level)
    {
        ...
        if (nameValidationCode == 0)
        {
            ...
            // A freshly created character must start with an empty equipment
            // container. Character ids can be reused after a soft deletion,
            // while the persistent container cache still holds the old slots.
            // Without clearing it, AddOrMoveExistingItem redirects every new
            // body part to slot 0 and EquipmentContainer rejects it as Head.
            character.Inventory.Equipment.Wipe();
