...
public class SpawnManager : Singleton<SpawnManager>
{
    ...
    private void AddExactNpcSpawner(NpcSpawner npcSpawner)
    {
        var spawners = new List<NpcSpawner> { npcSpawner };
        _npcSpawners[(byte)npcSpawner.Position.WorldId].Add(_nextId++, spawners);
    }
    ...
    public void Load()
    {
        ...
        foreach (var world in worlds)
        {
            ...
            // Load NPC spawns exported by the zone server. The .g spawnerType is the
            // base.npc_spawners.id; spawnerId is only the placement instance id.
            var loadedGSpawners = LoadNpcSpawnersG(world, worldPath);
            var jsonFileName = Path.Combine(worldPath, "npc_spawns.json");

            if (loadedGSpawners > 0)
            {
                Logger.Info($"Loaded {loadedGSpawners} NPC placements from zone_server .g files for {world.Name}");
            }
    ...
    private int LoadNpcSpawnersG(global::AAEmu.Game.Models.Game.World.World world, string worldPath)
    {
        var directory = Path.Combine(worldPath, "npc_spawners");
        if (!Directory.Exists(directory))
            return 0;

        var loaded = 0;
        var missingTemplates = new HashSet<uint>();
        foreach (var fileName in Directory.EnumerateFiles(directory, "*.g", SearchOption.TopDirectoryOnly))
        {
            if (!uint.TryParse(Path.GetFileNameWithoutExtension(fileName), out var zoneId))
            {
                Logger.Warn($"Skipping NPC spawn file with non-numeric zone name: {fileName}");
                continue;
            }

            foreach (var placement in NpcSpawnersGParser.Parse(fileName))
            {
                var template = NpcGameData.Instance.GetNpcSpawnerTemplate(placement.TemplateId);
                if (template == null)
                {
                    missingTemplates.Add(placement.TemplateId);
                    continue;
                }
                if (!template.ActivationState || template.Npcs.Count == 0)
                    continue;

                if (!ConvertPlacementToWorld(world, zoneId, placement))
                    continue;
                var initialPoint = placement.Points.Count > 0
                    ? placement.Points[0].Position
                    : placement.Triangles.Count > 0 ? placement.Triangles[0].V1 : Vector3.Zero;
                if (initialPoint == Vector3.Zero && placement.Points.Count == 0 && placement.Triangles.Count == 0)
                    continue;

                var unitId = template.Npcs.FirstOrDefault(member => member.MemberType == "Npc")?.MemberId ?? 0;
                if (unitId == 0)
                {
                    var group = template.Npcs.FirstOrDefault(member => member.MemberType == "NpcGroup");
                    unitId = group == null ? 0 : NpcGameData.Instance.GetNpcGroupMembers(group.MemberId).FirstOrDefault()?.NpcId ?? 0;
                }
                ...
                var spawner = new NpcSpawner
                {
                    ...
                    Template = template,
                    Placement = placement,
                    FollowPath = placement.Paths.OrderBy(path => path.PointNo).FirstOrDefault()?.Name,
                    Position = new WorldSpawnPosition
                    {
                        ...
                        Y = initialPoint.Y,
                        Z = initialPoint.Z,
                        Yaw = placement.Points.FirstOrDefault()?.ZRotation ?? 0f
                    }
                };
                spawner.NpcSpawnerIds.Add(placement.TemplateId);
                AddExactNpcSpawner(spawner);
                loaded++;
            }
        }

        if (missingTemplates.Count > 0)
            Logger.Warn($"Skipped NPC placements referencing {missingTemplates.Count} spawner types absent from base.sqlite3 in {world.Name}");
        return loaded;
    }

    private static bool ConvertPlacementToWorld(global::AAEmu.Game.Models.Game.World.World world, uint zoneId, NpcSpawnerPlacement placement)
    {
        if (!world.XmlWorldZones.TryGetValue(zoneId, out var zone))
        {
            Logger.Warn($"Skipping NPC placement zone {zoneId}: zone is absent from {world.Name}/world.xml");
            return false;
        }

        var offset = new Vector3(zone.OriginX * 1024f, zone.OriginY * 1024f, 0f);
        ...
        foreach (var anchor in placement.Anchors)
