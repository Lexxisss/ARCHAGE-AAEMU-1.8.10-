...
public class ItemManager : Singleton<ItemManager>
{
    ...
    public void Load()
    {
        ...
        */
        ...
        using (var connection = SQLite.CreateConnection("Data", SQLite.TargetClientDatabase))
        {
            ...
            // item_cap_scales is absent from every available client/server database.
            // Keep the collection empty until its target 10.8 replacement is identified.
    ...
    public (int, int, int) Save(MySqlConnection connection, MySqlTransaction transaction, uint? ownerId = null)
    {
        var deleteCount = 0;
        var updateCount = 0;
        ...
        using (var command = connection.CreateCommand())
        {
            ...
            lock (_allPersistantContainers)
            {
                ...
                {
                    if (ownerId.HasValue && c.OwnerId != ownerId.Value)
                        continue;
        ...
        using (var command = connection.CreateCommand())
        {
            ...
            lock (_allItems)
            {
                foreach (var entry in _allItems)
                {
                    ...
                    if (ownerId.HasValue && item.OwnerId != ownerId.Value)
                        continue;
                    ...
                    if (command.ExecuteNonQuery() < 1)
                    {
                        ...
                        if (ownerId.HasValue)
                            throw new InvalidOperationException($"MySQL did not persist item {item.Id} for owner {ownerId.Value}");
                    }
                    else
                    {
                        item.IsDirty = false;
