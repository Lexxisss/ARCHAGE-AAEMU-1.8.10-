...
public class SkillManager : Singleton<SkillManager>, ISkillManager
{
    ...
    private Dictionary<uint, HashSet<uint>> _taggedNpcs;
    ...
    */
    ...
    {
        return _startAbilitySkills.TryGetValue(ability, out var skills) ? skills : new List<SkillTemplate>();
    }
    ...
    public bool IsNpcInTag(uint npcTemplateId, uint tagId)
    {
        return _taggedNpcs.TryGetValue(tagId, out var npcIds) && npcIds.Contains(npcTemplateId);
    }
    ...
    public void Load()
    {
        ...
        */
        ...
        _taggedNpcs = new Dictionary<uint, HashSet<uint>>();
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (var reader = new SQLiteWrapperReader(sqliteReader))
                {
                    while (reader.Read())
                    {
                        ...
                        template.UseConditionBits = (SkillUseCondition)reader.GetInt64("use_condition_bits", 0);
                        ...
                        template.CooldownTagId = ParseNullableInt32(value);
                        value = reader.GetString("skill_controller_id", "0");
                        template.SkillControllerId = ParseNullableUInt32(value);
                        ...
                        // Modern client databases expose these conditions as explicit
                        // columns. Apply them after the legacy bit field so the target
                        // database values win instead of being overwritten by it.
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (var reader = new SQLiteWrapperReader(sqliteReader))
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            Logger.Warn("Default skill {0} is missing from the merged skills table; skipping it", id);
                            continue;
                        }
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (var reader = new SQLiteWrapperReader(command.ExecuteReader()))
                {
                    while (reader.Read())
                    {
                        ...
                        template.MainhandToolId = ParseNullableUInt32(value);
                        value = reader.GetString("offhand_tool_id", "0");
                        template.OffhandToolId = ParseNullableUInt32(value);
                        value = reader.GetString("tick_mainhand_tool_id", "0");
                        template.TickMainhandToolId = ParseNullableUInt32(value);
                        value = reader.GetString("tick_offhand_tool_id", "0");
                        template.TickOffhandToolId = ParseNullableUInt32(value);
...
LEFT JOIN formula_funcs ff ON dum.func_type = 'FormulaFunc' AND ff.id = dum.func_id";
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (var reader = new SQLiteWrapperReader(command.ExecuteReader()))
                {
                    while (reader.Read())
                    {
                        ...
                        // For easier debugging and deterministic per-relation processing.
                        template.EffectId = effectId;

                        if (!_types.TryGetValue(effectId, out var type))
                        {
                            Logger.Warn(
                                "Skill effect relation {0} references missing effect type {1}; relation skipped",
                                template.Id,
                                effectId);
                            continue;
                        }

                        if (_effects.TryGetValue(type.Type, out var typedEffects)
                            && typedEffects.TryGetValue(type.ActualId, out var effectTemplate))
                        ...
                        template.StartCombatResource = reader.GetInt32("start_combat_resource");
                        template.EndCombatResource = reader.GetInt32("end_combat_resource");
                        template.TargetCombatResourceId = reader.GetUInt32("target_combat_resource_id", 0);
                        template.ExecuteEffectOnFire = reader.GetBoolean("excute_effect_on_fire", true);
                        template.SourceBuffStackCountMin = reader.GetInt32("source_buff_stack_count_min");
                        template.SourceBuffStackCountMax = reader.GetInt32("source_buff_stack_count_max");
                        template.TargetBuffStackCountMin = reader.GetInt32("target_buff_stack_count_min");
                        template.TargetBuffStackCountMax = reader.GetInt32("target_buff_stack_count_max");
                        template.SourceExceptBuffStackCountMin = reader.GetInt32("source_except_buff_stack_count_min");
                        template.SourceExceptBuffStackCountMax = reader.GetInt32("source_except_buff_stack_count_max");
                        template.TargetExceptBuffStackCountMin = reader.GetInt32("target_except_buff_stack_count_min");
                        template.TargetExceptBuffStackCountMax = reader.GetInt32("target_except_buff_stack_count_max");
                        template.SynergyTextId = reader.GetUInt32("synergy_text", 0);
            ...
            {
                using var command = connection.CreateCommand();
                command.CommandText = "SELECT tag_id, npc_id FROM tagged_npcs";
                command.Prepare();
                using var reader = new SQLiteWrapperReader(command.ExecuteReader());
                while (reader.Read())
                {
                    ...
                    if (!_taggedNpcs.TryGetValue(tagId, out var npcIds))
                    {
                        npcIds = new HashSet<uint>();
                        _taggedNpcs.Add(tagId, npcIds);
                    }
            ...
            {
                Logger.Warn("Table tagged_npcs is missing, NPC-tagged skill effects will be skipped.");
            }
