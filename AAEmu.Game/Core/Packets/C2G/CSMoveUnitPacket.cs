...
public class CSMoveUnitPacket : GamePacket
{
    ...
    public override void Execute()
    {
        ...
        */
        ...
        var isFirstWorldMovement =
            _objId == character.ObjId && Connection.WorldEntryReady && !character.WorldEntryComplete;
            ...
            case UnitMoveType dmt:
                {
                    ...
                    */
                    ...
                    // The successful target entry starts publishing nearby objects
                    // immediately after the first local move and does not require an
                    // echo of that bootstrap movement to the local actor.
                    targetUnit.BroadcastPacket(
                        new SCOneUnitMovementPacket(_objId, dmt),
                        !isFirstWorldMovement);
        ...
        if (isFirstWorldMovement)
        {
            character.WorldEntryComplete = true;

            // NPCs are already published at the verified 0x0192 boundary.
            // First movement only completes the local-character visibility and
            // login hooks; publishing here again would duplicate NPC states.
            ...
            Logger.Info(
                "Completed first post-ready movement: characterId={0}, objId={1}",
                character.Id,
                character.ObjId);
        }
    }
