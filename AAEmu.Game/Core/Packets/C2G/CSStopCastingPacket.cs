...
public class CSStopCastingPacket : GamePacket
{
    ...
    public override async void Read(PacketStream stream)
    {
        ...
        var activeTask = Connection.ActiveChar.SkillTask;
        if (activeTask == null || activeTask.Skill.TlId != tlId)
        {
        ...
        if (activeTask is EndChannelingTask channelingTask)
            activeTask.Skill.Stop(Connection.ActiveChar, channelingTask._channelDoodad);
        else
            activeTask.Skill.Stop(Connection.ActiveChar);
    }
}
