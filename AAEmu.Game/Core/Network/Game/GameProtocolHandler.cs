...
public class GameProtocolHandler : BaseProtocolHandler
{
        ...
        try
        {
            ...
            while (stream != null && stream.Count > 0)
            {
                ...
                if (packetLen <= stream.Count)
                {
                    ...
                    LogCharacterEntryPacket(connection, level, type, stream2);
    ...
    private static void LogCharacterEntryPacket(GameConnection connection, byte level, ushort type, PacketStream stream)
    {
        if (level != 5 || type is not (
                CSOffsets.CSCreateCharacterPacket or
                CSOffsets.CSSelectCharacterPacket or
                CSOffsets.CSPostSelectCharacterPacket or
                CSOffsets.CSNotifyInGamePacket or
                CSOffsets.CSSpawnCharacterPacket or
                CSOffsets.CSInstanceLoadedPacket))
        {
            return;
        }

        const int maxLoggedBytes = 4096;
        var payloadLength = Math.Max(0, stream.Count - stream.Pos);
        var loggedLength = Math.Min(payloadLength, maxLoggedBytes);
        var payload = Convert.ToHexString(stream.Buffer, stream.Pos, loggedLength);
        Logger.Info(
            "CharacterEntryInput: accountId={0} state={1} level={2} opcode=0x{3:X3} payloadLen={4} payload={5}{6}",
            ...
            payloadLength,
            payload,
            loggedLength < payloadLength ? "...TRUNCATED" : string.Empty);
    }
}
