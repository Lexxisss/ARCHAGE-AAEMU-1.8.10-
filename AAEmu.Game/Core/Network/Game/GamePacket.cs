...
public abstract class GamePacket : PacketBase<GameConnection>
{
    ...
    public override PacketStream Encode()
    {
        //lock (Connection.WriteLock)
        {
                    ...
                    case 5:
                        {
                            ...
                            bodyCrc.Write(count)
                                .Write(TypeId)
                                .Write(this);
                            bodyLength = bodyCrc.Count - sizeof(byte) - sizeof(ushort);
            ...
            var logString = $"GamePacket: S->C type [{Level}:{TypeId:X3}] Body:[{bodyLength}] C:[{count}:{EncryptionManager.Instance.GetSCMessageCount(Connection.Id, Connection.AccountId)}] {ToString()?.Substring(23)}{Verbose()}";
            switch (LogLevel)
            {
