    ...
    public void Load(SqliteConnection connection, SqliteConnection connection2)
    {
        ...
        using (var command = connection.CreateCommand())
        {
            command.CommandText = @"SELECT
                    target.id,
                    COALESCE(target.tod, fallback.tod) AS tod,
                    COALESCE(target.first_wave_after, fallback.first_wave_after) AS first_wave_after,
                    COALESCE(target.target_npc_spawner_id, fallback.target_npc_spawner_id, 0) AS target_npc_spawner_id,
                    COALESCE(target.kill_npc_id, fallback.kill_npc_id, 0) AS kill_npc_id,
                    COALESCE(target.kill_npc_count, fallback.kill_npc_count, 0) AS kill_npc_count,
                    COALESCE(target.force_end_time, fallback.force_end_time) AS force_end_time,
                    COALESCE(target.tod_day_interval, fallback.tod_day_interval) AS tod_day_interval
                FROM tower_defs AS target
                LEFT JOIN client_fallback.tower_defs AS fallback ON fallback.id = target.id
                WHERE COALESCE(target.tod, fallback.tod) IS NOT NULL
                  AND COALESCE(target.first_wave_after, fallback.first_wave_after) IS NOT NULL
                  AND COALESCE(target.force_end_time, fallback.force_end_time) IS NOT NULL
                  AND COALESCE(target.tod_day_interval, fallback.tod_day_interval) IS NOT NULL";
            command.Prepare();
