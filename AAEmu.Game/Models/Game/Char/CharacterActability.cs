...
public class CharacterActability
{
    ...
    public void Send()
    {
        var initialActabilities = CharacterManager.Instance
            .GetActabilityTemplates()
            .Select(template => Actabilities.TryGetValue(template.Id, out var actability)
                ? actability
                : new Actability(template))
            .OrderBy(actability => actability.Id)
            .ToArray();

        Owner.SendPacket(new SCActabilityPacket(true, initialActabilities));
    }
