...
public partial class Character : Unit, ICharacter
{
    ...
    public CharacterVisualOptions VisualOptions { get; set; } = new();
    ...
    public ulong PendingSelectiveItemId { get; set; }
    public uint PendingSelectiveSkillId { get; set; }
    public DateTime PendingSelectiveItemExpiresAt { get; set; }
    ...
    /// <summary>
    /// True after the target client has started publishing movement for this
    /// character. Until then the actor is still crossing the lobby/world
    /// boundary and must not receive periodic active-world point updates.
    /// </summary>
    public bool WorldEntryComplete { get; set; }
    ...
    public void OnZoneChange(uint lastZoneKey, uint newZoneKey)
    {
        ...
        // During lobby character selection OnZoneChange is also used to
        // initialize zone buffs. Do not acknowledge the zone chat there:
        // the target client expects all three 0x01E5 acknowledgements at
        // the 0x0192 world-permission boundary.
...
#pragma warning restore CA1508 // Avoid dead conditional code
    ...
    public override void Regenerate()
    {
        ...
        // Do not interleave periodic 0x0187 packets with the world-entry
        // bootstrap. The first points packet is sent explicitly after 0x0192;
        // periodic updates resume after visibility has completed.
        if (IsVisible && WorldEntryComplete)
            ...
            using (var transaction = sqlConnection.BeginTransaction())
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        Logger.Error(
                            "Character save returned failure and was rolled back for {0} - {1}",
                            Id,
                            Name);
                    }
                }
        ...
        try
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                command.CommandText =
                    ...
                    "`money`,`money2`,`honor_point`,`vocation_point`,`crime_point`,`crime_record`,`hostile_faction_kills`,`pvp_honor`," +
            ...
            // Character creation must be atomic from the lobby's point of view: persist the
            // new owner's containers and starting items in this same transaction before 0x0A1
            // is sent. The regular world save tick still handles global dirty items.
            var savedItems = ItemManager.Instance.Save(connection, transaction, Id);
            Logger.Info(
                "Saved character inventory transactionally: charId={0}, items={1}, containers={2}",
                Id,
                savedItems.Item1,
                savedItems.Item3);
            result = true;
    ...
    {
        if (this != character) // Never send local SCUnitState back to self
        {
            ...
            character.SendPacket(new SCUnitModelPostureChangedPacket(
                this, BaseUnitType.Character, ModelPostureType.None));
        }
        /*
    ...
    /// <summary>
    /// Serializes the lobby character descriptor consumed by the 10.8.1.0 client.
    /// This is assembled from live character state; it is not a captured packet replay.
    /// The layout is confirmed by the working 1.8 reference capture and by the target
    /// x2game.dll allocating an 0x2020-byte decoded character descriptor with 35 item slots.
    /// </summary>
    public PacketStream WriteCharacterList1810(PacketStream stream)
    {
        var startOffset = stream.Count;

        stream.Write(Id);
        stream.Write(0u); // high character-id/reserved field in the 1.8 wire descriptor
        stream.Write(Name);
        stream.Write((byte)Race);
        stream.Write((byte)Gender);
        stream.Write(Level);
        stream.Write(0L); // heirExp is an int64 in the compiled target DLL
        stream.Write(Hp);
        stream.Write(Mp);
        stream.Write(Transform.ZoneId);
        ...
        WriteCharacter1810Tail(stream);

        Logger.Info(
            "Character lobby descriptor 1.8: charId={0}, name={1}, length={2}",
            Id,
            Name,
    ...
    private void InventoryEquip1810(PacketStream stream)
    {
        var items = Inventory.Equipment.GetSlottedItemsList();
        ulong validFlags = 0;
        for (var index = 0; index < items.Count && index < 35; index++)
        {
            if (items[index] != null)
    ...
    private void WriteCharacter1810Tail(PacketStream stream)
    {
        // Exact field order from target x2game.dll serializer 0x3996B620.
        // The successful reference capture's preceding 20 bytes belong to FaceModel,
        // not to this character tail.
        stream.Write(DeadCount);
        stream.Write(DeadTime);
        stream.Write(RezWaitDuration);
        stream.Write(0); // specialRezWaitDuration, not present in the legacy MySQL schema
        stream.Write(RezTime);
        stream.Write(RezPenaltyDuration);
        ...
        stream.Write(Money2);
        stream.Write(0L);
        stream.Write(AutoUseAAPoint ? (byte)1 : (byte)0);
        stream.Write(PrevPoint);
        stream.Write(Point);
        stream.Write(Gift);
        stream.Write(Updated);
        stream.Write(ForceNameChange);
        stream.Write((ushort)16);
        stream.Write(Guid.ToByteArray());

        // Exact target labor block from compiled serializer 0x39966330.
        stream.Write((int)LaborPower);
        stream.Write((int)LocalLaborPower);
        stream.Write(ConsumedLaborPower);
        stream.Write(LaborPowerModified);
        stream.Write(BmPoint);
        stream.Write((int)RechargedLp);
        stream.Write(RechargeResetTime);
    }
