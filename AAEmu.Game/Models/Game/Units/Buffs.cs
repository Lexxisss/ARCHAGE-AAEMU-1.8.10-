...
public class Buffs : IBuffs
{
    ...
    /// <summary>
    /// A copy of the effects to walk over safely.
    /// </summary>
    /// <remarks>
    /// The list is shared between the world tick, skill effects and scheduled tasks, and copying
    /// it without the lock is not safe: the copy reads the count first and fills the array after,
    /// so a buff expiring in between leaves the copy short and throws. That was a live crash -
    /// a dispel firing a damage trigger while the same list was being walked.
    ///
    /// Taking a copy rather than holding the lock for the whole walk is deliberate: the callers
    /// end up removing effects as they go, and several of them reach back into this object.
    /// </remarks>
    private Buff[] Snapshot()
    {
        lock (_lock)
        {
    ...
    public bool CheckBuffTag(uint tagId)
    {
        ...
        foreach (var effect in Snapshot())
            if (effect?.Template != null && buffs.Contains(effect.Template.BuffId))
                return true;
        return false;
    }
    ...
    {
        return Snapshot().Where(b => b.Template.RequireBuffId == buffId);
    }
    ...
    public int GetBuffCountByTag(uint tagId)
    {
        var taggedBuffs = SkillManager.Instance.GetBuffsByTagId(tagId);
        if (taggedBuffs is not { Count: > 0 })
            return 0;

        var taggedBuffSet = new HashSet<uint>(taggedBuffs);
        lock (_lock)
            return _effects.Count(effect => effect?.Template?.BuffId > 0 && taggedBuffSet.Contains(effect.Template.BuffId));
    }
    ...
    public void AddBuff(uint buffId, BaseUnit caster)
    {
        ...
        AddBuff(new Buff(owner, caster, casterObj, template, null, DateTime.UtcNow));
    }

    public void AddBuff(Buff buff, uint index = 0, int forcedDuration = 0)
    {
        if (buff?.Template == null || buff.Caster == null || buff.Owner == null)
            return;
        ...
        lock (_lock)
        {
            ...
            {
                var bufft = SkillManager.Instance.GetBuffTemplate(buff.Template.BuffId) ?? buff.Template;
        ...
        {
            var finalTemplate = SkillManager.Instance.GetBuffTemplate(finalToleranceBuffId);
            if (finalTemplate != null)
                AddBuff(new Buff(buff.Owner, buff.Caster, buff.SkillCaster, finalTemplate, buff.Skill, DateTime.UtcNow));
        }
    }
    ...
    public void RemoveEffect(Buff buff)
    {
        ...
        {
            var owner = GetOwner();
            if (owner == null || buff?.Template == null || !_effects.Contains(buff))
                return;

            // SetInUse(false) completes the buff lifecycle and calls this method again
            // from Buff.StopEffectTask after setting the state to Finished. Only the
            // finished re-entry detaches caches, otherwise modifiers are removed twice.
            if (buff.State != EffectState.Finished)
            {
            ...
            owner.SkillModifiersCache.RemoveModifiers(buff.Template.BuffId);
            owner.BuffModifiersCache.RemoveModifiers(buff.Template.BuffId);
            owner.CombatBuffs.RemoveCombatBuff(buff.Template.BuffId);
    ...
    public void RemoveEffect(uint templateId, uint skillId)
    {
        ...
        foreach (var effect in Snapshot())
        {
            if (effect?.Template != null &&
                effect.Template.Id == templateId &&
                effect.Skill?.Template?.Id == skillId)
            {
    ...
    public void RemoveEffect(uint index)
    {
        ...
        var effect = Snapshot().FirstOrDefault(candidate => candidate?.Index == index);
        effect?.SetInUse(false, false);
    }
    ...
    public void RemoveBuff(uint buffId)
    {
        ...
        var effect = Snapshot().FirstOrDefault(candidate => candidate?.Template?.BuffId == buffId);
        effect?.SetInUse(false, false);
    }
    ...
    {
        return Snapshot().Where(e => e.Template.DamageAbsorptionTypeId > 0);
    }
