...
public class BaseUnit : GameObject, IBaseUnit
{
    ...
    /// <summary>
    /// Sends the active, non-passive buff state required when this unit first
    /// becomes visible to a client. Passive buffs are already represented by
    /// SCUnitState and must not be duplicated here.
    /// </summary>
    public void SendActiveBuffs(Character character)
    {
        if (character == null || Buffs == null)
            return;

        var goodBuffs = new System.Collections.Generic.List<Buff>();
        var badBuffs = new System.Collections.Generic.List<Buff>();
        var hiddenBuffs = new System.Collections.Generic.List<Buff>();
        Buffs.GetAllBuffs(goodBuffs, badBuffs, hiddenBuffs);

        foreach (var buff in goodBuffs.Concat(badBuffs).Concat(hiddenBuffs))
        {
            if (buff == null || buff.Passive || !buff.InUse)
                continue;

            character.SendPacket(new SCBuffCreatedPacket(buff));
            character.SendPacket(new SCBuffUpdatedPacket(buff));
        }
    }
