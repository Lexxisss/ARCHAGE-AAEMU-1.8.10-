...
public class PacketStream : ICloneable, IComparable
{
    ...
    public long[] ReadPiscW(int count)
    {
        if (count <= 0)
        ...
        while (offset < count)
        {
            var groupSize = Math.Min(4, count - offset);
            var group = ReadPisc(groupSize);
            Array.Copy(group, 0, values, offset, groupSize);
            offset += groupSize;
        }
    ...
    public PacketStream WritePiscW(int count, params long[] values)
    {
        ...
        {
            var groupSize = Math.Min(4, count - offset);
            var group = new long[groupSize];
            Array.Copy(values, offset, group, 0, groupSize);
            WritePisc(group);
            offset += groupSize;
        }
